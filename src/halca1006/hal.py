...
class HALProtocol(object):  # pylint: disable=too-many-instance-attributes, too-many-public-methods
    ...
    * Use disconnect() to disconnect from the HAL CA1006 unit
    ...
    def _txrx(self, send_msg):
        self._easy_log(HAL_LOG_DEBUG, f"_txrx: Obtaining lock for {self._lock}")
        with self._lock:
            self._easy_log(HAL_LOG_DEBUG, f"_txrx: Lock obtained for {self._lock}")
            self._sock_send(send_msg)
            rcv_msg = self._read_input_buffer()

        self._easy_log(HAL_LOG_DEBUG, f"_txrx: Lock released for {self._lock}")
        return rcv_msg

    def _sock_recv(self, bufsize):
        for i in range(MAX_RECONNECTS):
            try:
                rlist, [], xlist = select.select(
                    [self._sock], [], [self._sock], self._select_interval
                )
                if self._sock in xlist:
                    self._easy_log(
                        HAL_LOG_ERROR,
                ...
                self._easy_log(
                    HAL_LOG_ERROR,
                    f"Connection Error ({err}) on recv. " f"Reconnect attempt {i+1}.",
                )
                self._reconnect()
                return b""
            except:
    ...
    def _sock_send(self, buf):
        ...
        for i in range(MAX_RECONNECTS):
            ...
            if inter_tx_time < HAL_MSG_INTERVAL:
                self._easy_log(
                    HAL_LOG_DEBUG, f"Short inter-transmission time ({inter_tx_time} s)."
                )
                self._easy_log(
                    HAL_LOG_DEBUG, f"Sleeping for {HAL_MSG_INTERVAL - inter_tx_time} s."
                )
                time.sleep(HAL_MSG_INTERVAL - inter_tx_time)

            self._easy_log(HAL_LOG_DEBUG, f"_sock_send():{self._name}: Sending {buf}")
            self._last_tx_t = time.time()
            try:
                ...
                self._easy_log(
                    HAL_LOG_ERROR,
                    f"Connection Error ({err}) on send. " f"Reconnect attempt {i+1}.",
                )
                self._reconnect()
                return 0
            except:
    ...
    def _set_echo(self, echo):
        ...
        self._easy_log(
            HAL_LOG_DEBUG, f"set_echo():{self._name}: change echo to {echo}."
        )
        if echo == "0":
            send_msg = ECHO_OFF
        else:
        ...
        rcv_msg = self._read_input_buffer()
        self._easy_log(HAL_LOG_DEBUG, f"set_echo():{self._name}: Bytes read: {rcv_msg}")
        return True
    ...
    def _read_input_buffer(self):
        ...
        while not all_input_read:
            ready_to_read, [], in_error = select.select(
                [self._sock], [], [self._sock], self._select_interval
            )
            if len(in_error) != 0:
                self._easy_log(
                ...
                self._easy_log(HAL_LOG_DEBUG, f"Read data:{self._name}: {reply_msg}")
                self._easy_log(HAL_LOG_DEBUG, "Data length = %d bytes ", len(reply_msg))
                if not reply_msg:
                    self._easy_log(
                        HAL_LOG_ERROR,
    ...
    def connect(self):
        ...
        try:
            self._sock.setsockopt(
                socket.SOL_SOCKET, socket.SO_KEEPALIVE, self._keepalive
            )
            c = self._sock.connect((self._host, self._port))
            self._easy_log(
                HAL_LOG_DEBUG,
            ...
            if self.is_connected():
                self._easy_log(
                    HAL_LOG_WARNING, f"connect():{self._name}: Already connected."
                )
                return True
    ...
    def disconnect(self):
        """Disconnect from a HAL CA1006 multi-zone amplifier"""
        if not self.is_connected():
            return HAL_ERR_NO_CONN

        self._easy_log(HAL_LOG_DEBUG, "disconnect(): Disconnecting.")
    ...
    def set_node_status(self, zone, power, source, volume, mute):  # pylint: disable=too-many-arguments, too-many-positional-arguments
        ...
        self._easy_log(
            HAL_LOG_DEBUG, f"set_node_status():{self._name}: Bytes read: {rcv_msg}"
        )
    ...
    def set_power(self, zone, power):
        ...
        self._easy_log(
            HAL_LOG_DEBUG, f"set_power():{self._name}: Bytes read: {rcv_msg}"
        )
    ...
    def set_volume(self, zone, volume):
        ...
        self._easy_log(
            HAL_LOG_DEBUG, f"set_volume():{self._name}: Bytes read: {rcv_msg}"
        )
    ...
    def set_source(self, zone, source):
        ...
        self._easy_log(
            HAL_LOG_DEBUG, f"set_source():{self._name}: Bytes read: {rcv_msg}"
        )
    ...
    def all_on_off(self, power):
        ...
        Note that the all on function is not supported by the HAL CA1006, 
        although it does support the all off.
        """

        self._easy_log(HAL_LOG_DEBUG, f"all_on_off():{self._name}: Power {power}.")

        if power != "0":
            self._easy_log(
                HAL_LOG_WARNING, f"all_on_off():{self._name}: Only supports all off."
            )
            return False
        ...
        self._easy_log(
            HAL_LOG_DEBUG, f"all_on_off():{self._name}: Bytes read: {rcv_msg}"
        )
    ...
    def set_mute(self, zone, mute):
        ...
        self._easy_log(HAL_LOG_DEBUG, f"set_mute():{self._name}: Bytes read: {rcv_msg}")
    ...
    def _get_version(self):
        ...
        self._easy_log(HAL_LOG_DEBUG, f"_get_version():{self._name}.")
        ...
        self._easy_log(
            HAL_LOG_DEBUG, f"_get_version():{self._name}: Bytes read: {rcv_msg}"
        )
        index = self._find_signature(rcv_msg, HAL_SUCCESS_MSG[GET_VERSION])
        if index != -1:
            self._easy_log(
                HAL_LOG_DEBUG, f"_get_version():{self._name}: Success! Index {index}."
            )
    ...
    def _get_am6_status(self):
        ...
        """
        self._easy_log(HAL_LOG_DEBUG, "_get_am6_status()")
        ...
        self._easy_log(
            HAL_LOG_DEBUG, f"_get_node_status():{self._name}: Bytes read: {rcv_msg}"
        )

    def _get_node_status(self, zone):
        """Get node status for the zone"""

        self._easy_log(HAL_LOG_DEBUG, f"_get_node_status():{self._name}: zone {zone}.")
        ...
        if index != -1:
            ...
            self._easy_log(
                HAL_LOG_DEBUG, f"_get_node_status():{self._name}: Status is {status}."
            )
        else:
            self._easy_log(
                HAL_LOG_ERROR,
    ...
    def get_node_status(self, zone):
        ...
        self._easy_log(HAL_LOG_DEBUG, f"get_node_status():{self._name}: zone {zone}.")
        return self._node_status[int(zone)]

    def get_zone_info(self, zone):
        """Get all relevant info for the zone"""

        self._easy_log(HAL_LOG_DEBUG, f"get_zone_info():{self._name}: zone {zone}.")
        ...
        self._easy_log(
            HAL_LOG_DEBUG, f"get_zone_info():{self._name}: Bytes read: {rcv_msg}"
        )
        index = self._find_signature(rcv_msg, HAL_SUCCESS_MSG[GET_RA_STATUS])
        if index != -1:
            self._easy_log(
                HAL_LOG_DEBUG, f"get_zone_info():{self._name}: Success! Index {index}."
            )
            index += len(HAL_SUCCESS_MSG[GET_RA_STATUS])
            data = rcv_msg[index : index + 14]
            split_data = data.split(b" ")
            self._easy_log(
                HAL_LOG_DEBUG, f"get_zone_info():{self._name}: Data ({data})."
            )
            self._easy_log(
                HAL_LOG_DEBUG,
            ...
            self._easy_log(
                HAL_LOG_DEBUG, f"get_zone_info():{self._name}: Status ({status})."
            )
            source = data[3:5]
            self._source[int(zone)] = source.decode()
            self._easy_log(
                HAL_LOG_DEBUG, f"get_zone_info():{self._name}: Source ({source})."
            )
            volume = data[6:8]
            self._volume[int(zone)] = volume.decode()
            self._easy_log(
                HAL_LOG_DEBUG, f"get_zone_info():{self._name}: Volume ({volume})."
            )
            video = data[9:11]
            self._easy_log(
                HAL_LOG_DEBUG, f"get_zone_info():{self._name}: Video ({video})."
            )
            mute = data[12:14]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Expose HAL CA1006 multi-zone amplifier"
    )
    parser.add_argument(
        "--log_level",
    ...
    parser.add_argument(
        "--hal_host",
        help="The hostname or IP address of the HAL CA1006. " "Defaults to localhost.",
        default="localhost",
    )
    parser.add_argument(
        "--hal_port",
        help="The HAL CA1006 IP port. Defaults to 7000.",
        type=int,
        default=7000,
    )
    ...
    # Establish logging
    main_logger = logging.getLogger(sys.argv[0])
    main_logger.setLevel(LOGGING_LEVEL[main_args.log_level])

    # create handler and set level
    ch = logging.StreamHandler()
    ch.setLevel(LOGGING_LEVEL[main_args.log_level])
    ...
    main_logger.info("Obtaining HAL client")
    hal = HALProtocol(main_args.hal_host, main_args.hal_port)
    main_logger.info("Enabling logger")
    hal.enable_logger(main_logger)
    s = hal.socket()
    main_logger.info("HAL socket fd is %d.", s.fileno())
    ...
    while True:
        ...
        if cmd == "1":
            main_logger.info("OK. Connecting..")
            hal.connect()
        elif cmd == "2":
            main_logger.info("OK. Checking if connected")
            if hal.is_connected():
                print(f"Yes, we are connected via socket {s}.\n")
            else:
                print("No, we are not connected.\n")
        elif cmd == "3":
            main_logger.info("Set Power Test")
            main_zone = input("Enter zone [1-6]: ")
            main_power = input("Enter Power mode 0=Off, 1=On: ")
            hal.set_power(main_zone, main_power)
        elif cmd == "4":
            main_logger.info("Set Volume Test")
            main_zone = input("Enter zone [1-6]: ")
            main_volume = input("Enter Volume [0-100]: ")
            hal.set_volume(main_zone, main_volume)
        elif cmd == "5":
            main_logger.info("Set Source Test")
            main_zone = input("Enter zone [1-6]: ")
            main_source = input("Enter Source [1-8]: ")
            hal.set_source(main_zone, main_source)
        elif cmd == "6":
            main_logger.info("All on/off test")
            main_power = input("Enter Power mode 0=Off, 1=On: ")
            hal.all_on_off(main_power)
        elif cmd == "7":
            main_logger.info("Mute test")
            main_zone = input("Enter zone [1-6]: ")
            main_mute = input("Enter Mute 0=Off, 1=On: ")
            hal.set_mute(main_zone, main_mute)
        elif cmd == "8":
            main_logger.info("Get Zone Info Test")
            main_zone = input("Enter zone [1-6]: ")
            hal.get_zone_info(main_zone)
        elif cmd == "9":
            main_logger.info("Get Power Test")
            main_zone = input("Enter zone [1-6]: ")
            print(f"Zone {main_zone} power is {hal.get_power(main_zone)}.")
        elif cmd == "10":
            main_logger.info("Get Source Test")
            main_zone = input("Enter zone [1-6]: ")
            print(f"Zone {main_zone} source is {hal.get_source(main_zone)}.")
        elif cmd == "11":
            main_logger.info("Get Volume Test")
            main_zone = input("Enter zone [1-6]: ")
            print(f"Zone {main_zone} volume is {hal.get_volume(main_zone)}.")
        elif cmd == "12":
            main_logger.info("Get Mute Test")
            main_zone = input("Enter zone [1-6]: ")
            print(f"Zone {main_zone} mute is {hal.get_mute(main_zone)}.")
        elif cmd == "13":
            main_logger.info("Set Echo")
            main_echo = input("Enter echo [0,1]: ")
            hal._set_echo(main_echo)  # pylint: disable=protected-access
        elif cmd == "14":
            main_logger.info("Get Version")
            main_version = hal.get_version()
            print(f"{main_version}")
        elif cmd == "15":
            main_logger.info("Set Node Status Test")
            main_zone = input("Enter zone [1-6]: ")
            main_power = input("Enter Power mode 0=Off, 1=On: ")
            main_source = input("Enter Source [1-8]: ")
            main_volume = input("Enter Volume [0-100]: ")
            main_mute = input("Enter Mute 0=Off, 1=On: ")
            hal.set_node_status(
                main_zone, main_power, main_source, main_volume, main_mute
            )
        elif cmd == "16":
            main_logger.info("Get Node Status Test")
            main_zone = input("Enter zone [1-6]: ")
            print(f"Zone {main_zone} status is {hal.get_node_status(main_zone)}")
        elif cmd == "17":
            main_logger.info("Set Select Interval")
            main_interval = input(f"Enter new interval > {HAL_SELECT_MIN_INTERVAL}:")
            hal.set_select_interval(main_interval)
        elif cmd == "18":
            main_logger.info("Get Select Interval")
            print(f"Select interval is {hal.get_select_interval()}s.")
        elif cmd == "20":
            main_logger.info("Show all")
            for izone in range(1, HAL_ZONES + 1):
                print(
                    f"Zone {izone} power is {hal._power[izone]}, "  # pylint: disable=protected-access
                    f"source is {hal._source[izone]}, "
                    f"volume is {hal._volume[izone]}, "  # pylint: disable=protected-access
                    f"mute is {hal._mute[izone]}."  # pylint: disable=protected-access
                )  # pylint: disable=protected-access
        elif cmd == "21":
            main_logger.info("Get all")
            hal._get_all_info()  # pylint: disable=protected-access
        else:
            break
